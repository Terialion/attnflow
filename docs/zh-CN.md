...
def _is_attention_layer(self, layer_name: str) -> bool:
    ...
    示例：只追踪特定的注意力实现
    """
    # 取模块名最后一段（如 layers.0.self_attn -> self_attn）
    module_name = layer_name.split(".")[-1]

    # 可按需处理驼峰（SelfAttention -> self_attention）
    # 省略具体实现细节...
    ...
    return module_name.lower() in (supported_names | custom_names)
```
